...
#define PT_TEST_REF TEST_ROOT "pt/reference/"

using namespace ChronusQ;

// run normal CQ job
inline void CQNORMALPT( std::string in, std::string ref ) {


#ifdef _CQ_GENERATE_TESTS

  RunChronusQ(TEST_ROOT + in + ".inp","STDOUT",
    PT_TEST_REF + ref, "", false);
...
// run CQ job from reference bin file
inline void CQBINPT( std::string in, std::string ref ) {

#ifdef _CQ_GENERATE_TESTS

  RunChronusQ(TEST_ROOT + in + ".inp","STDOUT",
    PT_TEST_REF + ref, "", true);

#else

  std::ifstream  src(PT_TEST_REF + ref, std::ios::binary);
...
inline void CQSCRPT( std::string in, std::string ref, std::string scr ) {

#ifdef _CQ_GENERATE_TESTS

  RunChronusQ(TEST_ROOT + in + ".inp","STDOUT",
    PT_TEST_REF + ref, PT_TEST_REF + scr, false);

#else

  // Run off a private copy of the reference scratch file: the job
  // writes to its scratch file, and tests run side by side.
  std::ifstream  scrSrc(PT_TEST_REF + scr, std::ios::binary);
...
inline void CQBINSCRPT( std::string in, std::string ref, std::string scr ) {

#ifdef _CQ_GENERATE_TESTS

  RunChronusQ(TEST_ROOT + in + ".inp","STDOUT",
    PT_TEST_REF + ref, PT_TEST_REF + scr, true);
...
inline void CQPTTEST( std::string in, std::string ref,
  bool readBin      = false,
  std::string scr   = "",
  double tol        = 1e-8,
//  bool checkOsc     = false,
  ...
  if( !readBin and scr=="" ) CQNORMALPT(in,ref);
  else if( readBin and scr=="" ) CQBINPT(in,ref);
  else if( !readBin and scr!="" ) CQSCRPT(in,ref,scr);
  else CQBINSCRPT(in,ref,scr);

#ifndef _CQ_GENERATE_TESTS

  SafeFile refFile(PT_TEST_REF + ref,  true,  true);
  SafeFile resFile(CQTestOut(in,".bin"),true);
  
  double xNS, yNS;
  std::cout << " * PERFORMING MSWFN ENERGY CHECK " << std::endl;
  std::cout << "PT_TEST_REF=" << PT_TEST_REF <<std::endl;
  ...
  if( checkOsc ) {
    ...
    std::cout << " * PERFORMING PT OSCILLATOR STRENGTH CHECK " << std::endl;
  ...
  if( checkDipLen ) {
    ...
    std::cout << " * PERFORMING PT DIPOLE (LEN) CHECK " << std::endl;
  ...
  if( checkQuadLen ) {
    ...
    std::cout << " * PERFORMING PT QUADRUPOLE (LEN) CHECK " << std::endl;
  ...
  if( checkOctLen ) {
    ...
    std::cout << " * PERFORMING PT OCTUPOLE (LEN) CHECK " << std::endl;
