...
GauXC::BasisSet<double> parse_basis( const GauXC::Molecule& mol,
                              std::string     fname,
                              GauXC::SphericalType   sph    ) {
  ...
  std::map<int, GauXC::BasisSet<double>> basis_shells;
  for( const auto& record : basis_records ) {
    ...
    GauXC::BasisSet<double> atom_basis;
    for( auto rec_it = record.begin()+1; rec_it != record.end(); ) {
      ...
      using prim_array = GauXC::Shell<double>::prim_array;
      using cart_array = GauXC::Shell<double>::cart_array;
      ...
      GauXC::SphericalType sph_use = l > 1 ? sph : GauXC::SphericalType(false);
      atom_basis.emplace_back( GauXC::Shell<double>(
        GauXC::PrimSize(nprim), GauXC::AngularMomentum(l), sph_use,
        alpha_arr, coeff_primary_arr, {0., 0., 0.}
      ));

      if( gencon )
        atom_basis.emplace_back( GauXC::Shell<double>(
          GauXC::PrimSize(nprim), GauXC::AngularMomentum(1), GauXC::SphericalType(false),
  ...
  for( auto iAt = 0; iAt < mol.size(); ++iAt ) {
    ...
    GauXC::BasisSet<double> atom_basis = basis_shells.at(atom.Z.get());
