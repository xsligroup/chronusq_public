...
namespace ChronusQ {
  ...
  void PostHartreeFock<MatsT,IntsT> :: runCube(std::vector<std::shared_ptr<CubeGen>> cubes, EMPerturbation &emPert) {
      ...
      if (cubeOptsPostHF.orbCube) {
        ...
        if(cubeOptsPostHF.whichMO == MO_CLASSES::ALL)
        {
        ...
        {
          OrbsToCube = cubeOptsPostHF.custom_orb_request;
        }
        ...
        // real and imaginary
        bool MagPhase = cubeOptsPostHF.MagnitudeAndPhase;
        if(MagPhase)
        {
