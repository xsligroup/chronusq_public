...
//    formW_abei();
  ...
  template <typename MatsT>
  void EOMIP_2h1p<MatsT>::buildDiag(MatsT * diag, const std::vector<double> &eps) const {
  ...
  template <typename MatsT>
  typename Davidson<MatsT>::VecsGen_t EOMIP_2h1p<MatsT>::EmptyDavidsonVectorBuilder(){
  ...
  template <typename MatsT>
  typename Davidson<MatsT>::LinearTrans_t EOMIP_2h1p<MatsT>::DavidsonResidualBuilder(EOMCCEigenVecType &eigenVecType){
      if (this->eomSettings.hbar_type == EOM_HBAR_TYPE::IMPLICIT) {
  ...
  template <typename MatsT>
  typename Davidson<MatsT>::LinearTrans_t EOMIP_2h1p<MatsT>::DavidsonPreconditionerBuilder(dcomplex * curEig, MatsT * eomDiag){
