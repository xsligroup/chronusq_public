...
namespace ChronusQ{
  ...
  template <typename MatsT>
  void EOMIP_3h2p<MatsT>::buildDiag(MatsT * diag, std::vector<double> eps) const {
  ...
  template <typename MatsT>
  typename Davidson<dcomplex>::VecsGen_t EOMIP_3h2p<MatsT>::EmptyDavidsonVectorBuilder(){
  ...
  template <typename MatsT>
  typename Davidson<dcomplex>::LinearTrans_t EOMIP_3h2p<MatsT>::DavidsonResidualBuilder(EOMCCEigenVecType &eigenVecType){
      if (this->eomSettings.hbar_type == EOM_HBAR_TYPE::IMPLICIT) {
  ...
  template <typename MatsT>
  typename Davidson<dcomplex>::LinearTrans_t EOMIP_3h2p<MatsT>::DavidsonPreconditionerBuilder(dcomplex * curEig, dcomplex * eomDiag){
      ...
      this->PCEOM = [this, eomDiag, curEig, PCsmall]( size_t nVec, SolverVectors<dcomplex> &V,
        ...
        for (size_t iVec = 0; iVec < nVec; iVec++) {
          ...
          TA::foreach_inplace(curB.get_tensor("TwoBody"), [iVec, curEig, Diag_ABIJK, this, PCsmall](TA::TensorZ &tile){
            const auto& lobound = tile.range().lobound();
            const auto& upbound = tile.range().upbound();
                  ...
                  for(x[3] = lobound[3]; x[3] < upbound[3]; ++x[3]){
                    ...
                    for(x[4] = lobound[4]; x[4] < upbound[4]; ++x[4]){
                      if (x[2] == x[4] or x[3] == x[4])
                         continue;
                      size_t k = x[4];
                      denom = curEig[iVec] - Diag_ABIJK[toCompoundT(a, b, i, j, k)];
                      if (std::abs(denom) >= PCsmall) tile[x] /= denom;
                    }
                  }
                }
