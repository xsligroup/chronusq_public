...
void NewOrbitalRotation<MatsT, IntsT>::computeOrbOrbHessianDiag(EMPerturbation & pert, 
...
#pragma omp parallel for schedule(static) default(shared)       
      for (auto a = 0ul; a < nSVirt; a++)
      for (auto i = 0ul; i < nInact; i++)
        SCR[i + a * nInact] = F1_aa[a] - F1_ii[i];
      
      SetMat('N', nInact, nSVirt, MatsT(1.), SCR, nInact, HP + nTOrb * nINCO, nTOrb);
      SetMat('C', nInact, nSVirt, MatsT(1.), SCR, nInact, HP + nINCO, nTOrb);
    } 
...
#pragma omp parallel for schedule(static) default(shared)       
      ...
      SetMat('N', nCorrO, nSVirt, MatsT(1.), SCR, nCorrO, HP + nTOrb * nINCO + nInact, nTOrb);
...
} // NewOrbitalRotation<MatsT>::computeOrbOrbHessianDiag
