...
namespace ChronusQ {
  ...
  void NEOSS<MatsT,IntsT>::formEXCGradient(EMPerturbation &pert) {
...
// TanGDD
  template <>
  void NEOSS<dcomplex,dcomplex>::formEXCGradient(EMPerturbation &pert) {

    // Obtaining subsystem as KS objects
    auto ess = getSubsystem<KohnSham>("Electronic");
    auto pss = getSubsystem<KohnSham>("Protonic");
    auto prot_ks = std::dynamic_pointer_cast<NEOKohnShamBuilder<dcomplex,dcomplex>>(this->fockBuilders["Protonic"].back());
    if(!ess or !pss or !prot_ks) CErr("Unsuccessful cast in NEOSS<MatsT,IntsT>::formEXCGradient()");
    ...
    {  
      ...
      dcomplex *SCRATCHNBNB = 
        CQMemManager::get().template malloc<dcomplex>(NTNB2); 
      dcomplex *SCRATCHNBNP = 
        CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB); 
      std::vector<std::vector<dcomplex*>> VEC_SCRATCHNBNB;
      for (size_t ic = 0; ic < nAtoms; ic++) {
        std::vector<dcomplex*> tmp = 
        {CQMemManager::get().template malloc<dcomplex>(NTNB2), CQMemManager::get().template malloc<dcomplex>(NTNB2), CQMemManager::get().template malloc<dcomplex>(NTNB2)};
        VEC_SCRATCHNBNB.emplace_back(tmp);
      }
      std::vector<std::vector<dcomplex*>> VEC_SCRATCHNBNP;
      for (size_t ic = 0; ic < nAtoms; ic++) {
        std::vector<dcomplex*> tmp = 
        {CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB)};
        VEC_SCRATCHNBNP.emplace_back(tmp);
      }
      std::vector<dcomplex*> VEC_SCRATCHNBNP2 = 
        {CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*NB)};
      ...
      // Decide if we need to allocate space for real part of the 
      // densities and copy over the real parts
      // TangDD: for GIAO we keep everything.
      ...
      // Note: Currently the 1PDM gradient is NOT used in exc energy gradient calculation. 
      //       We include this part of contribution in total pulay force
      std::vector<std::vector<dcomplex*>> Re1PDMGrad_scalar(nAtoms); 
      std::vector<std::vector<dcomplex*>> Re1PDMGrad_mz(nAtoms); 
#ifdef USE_ONEPDMGRAD
      for(auto ic = 0; ic < nAtoms; ic++) {
        for (auto xyz = 0; xyz < 3; xyz++ )
          Re1PDMGrad_scalar[ic].push_back(reinterpret_cast<dcomplex*>(ess->onePDMGrad[3*ic+xyz]->S().pointer()));
      }
      if( ess->onePDM->hasZ() ) {
        for(auto ic = 0; ic < nAtoms; ic++) {
          for(auto xyz = 0; xyz < 3; xyz++)
            Re1PDMGrad_mz[ic].push_back(reinterpret_cast<dcomplex*>(ess->onePDMGrad[3*ic+xyz]->Z().pointer()));
            }
        }
      }
#endif
      ...
      // Allocate scratch matrices
      dcomplex *AUX_SCRATCHNBNB =
        CQMemManager::get().template malloc<dcomplex>(aux_NTNB2);
      dcomplex *AUX_SCRATCHNBNP =
        CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB); 
      std::vector<std::vector<dcomplex*>> AUX_VEC_SCRATCHNBNB;
      for (size_t ic = 0; ic < nAtoms; ic++) {
        std::vector<dcomplex*> tmp =
          {CQMemManager::get().template malloc<dcomplex>(aux_NTNB2), CQMemManager::get().template malloc<dcomplex>(aux_NTNB2), CQMemManager::get().template malloc<dcomplex>(aux_NTNB2)};
        AUX_VEC_SCRATCHNBNB.emplace_back(tmp);
      }
      std::vector<std::vector<dcomplex*>> AUX_VEC_SCRATCHNBNP;
      for (size_t ic = 0; ic < nAtoms; ic++) {
        std::vector<dcomplex*> tmp = 
          {CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB)};
        AUX_VEC_SCRATCHNBNP.emplace_back(tmp);
      }
      std::vector<dcomplex*> AUX_VEC_SCRATCHNBNP2 = 
          {CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB), CQMemManager::get().template malloc<dcomplex>(NTNPPB*aux_NB)};
      ...
      std::shared_ptr<cqmatrix::PauliSpinorMatrices<dcomplex>> aux_Re1PDM;
      aux_Re1PDM = std::dynamic_pointer_cast<cqmatrix::PauliSpinorMatrices<dcomplex>>(pss->onePDM);

      // Note: Currently the 1PDM gradient is NOT used in exc energy gradient calculation. 
      //       We include this part of contribution in total pulay force
      std::vector<std::vector<dcomplex*>> aux_Re1PDMGrad_scalar(nAtoms); 
      std::vector<std::vector<dcomplex*>> aux_Re1PDMGrad_mz(nAtoms); 
#ifdef USE_ONEPDMGRAD
      for(auto ic = 0; ic < nAtoms; ic++) {
        for (auto xyz = 0; xyz < 3; xyz++ )
          aux_Re1PDMGrad_scalar[ic].push_back(reinterpret_cast<dcomplex*>(pss->onePDMGrad[3*ic+xyz]->S().pointer()));
      }
      if( pss->onePDM->hasZ() ) {
        for(auto ic = 0; ic < nAtoms; ic++) {
          for(auto xyz = 0; xyz < 3; xyz++)
            aux_Re1PDMGrad_mz[ic].push_back(reinterpret_cast<dcomplex*>(pss->onePDMGrad[3*ic+xyz]->Z().pointer()));
        }
      }
#endif
      // --------------------------------------------------------------------------------//
      ...
      auto vxcbuild = [&](size_t &res, std::vector<cart_t> &batch, 
        ...
        // Protonic system
        size_t aux_NBE = NBE_vec[1];
        dcomplex * aux_BasisEval = BasisEval_vec[1];
        std::vector<size_t> & aux_batchEvalShells = batchEvalShells_vec[1];
        std::vector<std::pair<size_t,size_t>> & aux_subMatCut = subMatCut_vec[1];
        ...
        size_t NDer = isGGA ? 4 : 1;
        dcomplex *BasisGradEval = BasisEval + NDer * NPts * NBE;

        size_t aux_NDer = epcisGGA ? 4 : 1;
        dcomplex *aux_BasisGradEval = aux_BasisEval + aux_NDer * NPts * aux_NBE;
#if VXC_DEBUG_LEVEL >= 1
        // TIMING
        auto topevalDen = std::chrono::high_resolution_clock::now();
#endif
        ...
        // local vector scratch pointers
        std::vector<std::vector<dcomplex*>> VEC_SCRATCHNBNB_loc;
        for (size_t ic = 0; ic < nAtoms; ic++) {
          std::vector<dcomplex*> tmp = 
            {VEC_SCRATCHNBNB[ic][0] + thread_id * NB2, VEC_SCRATCHNBNB[ic][1] + thread_id * NB2, VEC_SCRATCHNBNB[ic][2] + thread_id * NB2};
          VEC_SCRATCHNBNB_loc.emplace_back(tmp);
        ...
        std::vector<std::vector<dcomplex*>> VEC_SCRATCHNBNP_loc;
        for (size_t ic = 0; ic < nAtoms; ic ++) {
          std::vector<dcomplex*> tmp = 
            {VEC_SCRATCHNBNP[ic][0] + thread_id * NB*NPtsMaxPerBatch, VEC_SCRATCHNBNP[ic][1] + thread_id * NB*NPtsMaxPerBatch, VEC_SCRATCHNBNP[ic][2] + thread_id * NB*NPtsMaxPerBatch};
          VEC_SCRATCHNBNP_loc.emplace_back(tmp);
        ...
        std::vector<dcomplex*> VEC_SCRATCHNBNP2_loc = 
          {VEC_SCRATCHNBNP2[0] + thread_id * NB*NPtsMaxPerBatch, VEC_SCRATCHNBNP2[1] + thread_id * NB*NPtsMaxPerBatch, VEC_SCRATCHNBNP2[2] + thread_id * NB*NPtsMaxPerBatch};
        ...
        // --------------------------------------------------------------------------------//
        // aux local scratch pointers 
        dcomplex *  AUX_SCRATCHNBNB_loc = AUX_SCRATCHNBNB + thread_id * aux_NB2;
        dcomplex *  AUX_SCRATCHNBNP_loc = AUX_SCRATCHNBNP + thread_id * aux_NB*NPtsMaxPerBatch;         

        // aux local vector scratch pointers
        std::vector<std::vector<dcomplex*>> AUX_VEC_SCRATCHNBNB_loc;
        for (size_t ic = 0; ic < nAtoms; ic++) {
          std::vector<dcomplex*> tmp = 
            {AUX_VEC_SCRATCHNBNB[ic][0] + thread_id * aux_NB2, AUX_VEC_SCRATCHNBNB[ic][1] + thread_id * aux_NB2, AUX_VEC_SCRATCHNBNB[ic][2] + thread_id * aux_NB2};
          AUX_VEC_SCRATCHNBNB_loc.emplace_back(tmp);
        ...
        std::vector<std::vector<dcomplex*>> AUX_VEC_SCRATCHNBNP_loc;
        for (size_t ic = 0; ic < nAtoms; ic++) {
          std::vector<dcomplex*> tmp = 
            {AUX_VEC_SCRATCHNBNP[ic][0] + thread_id * aux_NB*NPtsMaxPerBatch, AUX_VEC_SCRATCHNBNP[ic][1] + thread_id * aux_NB*NPtsMaxPerBatch, AUX_VEC_SCRATCHNBNP[ic][2] + thread_id * aux_NB*NPtsMaxPerBatch};
          AUX_VEC_SCRATCHNBNP_loc.emplace_back(tmp);
        ...
        std::vector<dcomplex*> AUX_VEC_SCRATCHNBNP2_loc =
          {AUX_VEC_SCRATCHNBNP2[0] + thread_id * aux_NB*NPtsMaxPerBatch, AUX_VEC_SCRATCHNBNP2[1] + thread_id * aux_NB*NPtsMaxPerBatch, AUX_VEC_SCRATCHNBNP2[2] + thread_id * aux_NB*NPtsMaxPerBatch};
        ...
        // Integrate the VXC
        integrator.integrate<size_t>(vxcbuild, pert, true);
...
// Combine MPI results
      ...
      // TangDD: I have no idea of this statement. As long as numerical gradient is correct I'm ingorning this.
      //if( not std::is_same<MatsT,double>::value ) {
        for(size_t ic = 0; ic < nAtoms; ic++) {
          for(auto &X : Re1PDMGrad_scalar[ic]) CQMemManager::get().free(X);
          for(auto &X : Re1PDMGrad_mz[ic]) CQMemManager::get().free(X);
