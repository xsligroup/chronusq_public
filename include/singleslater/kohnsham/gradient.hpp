...
// Combine MPI results
  ...
  void KohnSham<dcomplex,dcomplex>::formEXCGradientInHouse(EMPerturbation &pert) {
...
#endif
    {
      ...
      for(auto ic = 0; ic < nAtoms; ic++) {
        for (auto xyz = 0; xyz < 3; xyz++ )//{
          Re1PDMGrad_scalar[ic].push_back(reinterpret_cast<dcomplex*>(this->onePDMGrad[3*ic+xyz]->S().pointer()));
          //prettyPrintSmart(std::cout,"1PDMGRAD DFT "+std::to_string(3*ic+xyz),Re1PDMGrad_scalar[ic][xyz],NB,NB,NB);}
      }
      ...
      if( this->onePDM->hasZ() ) {
        for(auto ic = 0; ic < nAtoms; ic++) {
          for(auto xyz = 0; xyz < 3; xyz++)
            Re1PDMGrad_mz[ic].push_back(reinterpret_cast<dcomplex*>(this->onePDMGrad[3*ic+xyz]->Z().pointer()));
        }
      }
#endif
