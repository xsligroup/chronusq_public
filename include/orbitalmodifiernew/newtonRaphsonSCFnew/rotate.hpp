...
template <template <typename, typename> class _SSTyp, typename MatsT, typename IntsT>
  void NewtonRaphsonSCF<_SSTyp,MatsT,IntsT>::saveRefMOs(vecMORef<MatsT>& mo) {
 ...
 */
template <template <typename, typename> class _SSTyp, typename MatsT, typename IntsT>
  void NewtonRaphsonSCF<_SSTyp,MatsT,IntsT>::computeGradient(vecMORef<MatsT>& mo) {

  // Compute the Gradient of orbital rotations
  if( this->singleSlaterSystem.computeNROrbGrad ) {
    // Compute user-defined gradient
    this->singleSlaterSystem.computeNROrbGrad(orbGrad);
  } else {
...
template <template <typename, typename> class _SSTyp, typename MatsT, typename IntsT>
  void NewtonRaphsonSCF<_SSTyp,MatsT,IntsT>::computeDiagHess(vecEPtr& eps){
 ...
 */
template <template <typename, typename> class _SSTyp, typename MatsT, typename IntsT>
  void NewtonRaphsonSCF<_SSTyp,MatsT,IntsT>::takeStep(MatsT* dx) {
