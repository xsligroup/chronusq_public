...
namespace ChronusQ {
    ...
    //helper function for sparse Davidson
  template <typename _F>
  bool Davidson<_F>::runSparseMicro() {
...
//
	...
	std::vector<_F> VAV(GetNumThreads() * nVCur * nDo ,_F(0.));
	this->formSubspaceMatrix_(nDo, nVCur, VRSend, VAV);

	// Construct submatrix of A
	for(size_t iVec = 0; iVec < nDo; iVec++) {
          for(size_t jVec = 0; jVec < nVCur; jVec++) {

	    SubA[(nVPrev + iVec) * MSS + jVec] = VAV[jVec + iVec * nVCur];

	    //populate tranpose of matrix, CHECK
	    if((nVPrev + iVec) * MSS != jVec) {
	      SubA[jVec * MSS + (nVPrev + iVec)] = SmartConj(SubA[(nVPrev + iVec) * MSS + jVec]);
	    }
	  }
	}
//      }
...
//          if (nVCur == 0) CErr("Not enough eigenpairs to proceed! ");
	...
	if(nRes > 0) {
	  SolverVectorsView<_F> SSend(*S, 0), RRecv(*R, 0);
	  this->linearTrans_(nRes, SSend, RRecv);
	}
      ...
      AVR->print(std::cout, "AVR");
        ...
        //for (auto i = 0ul; i < nRes; i++) {
	//  R->axpy(i, 1, -dcomplexTo_F(Eig[residualStates[i]]), *S, i);
        //}
...
//    if( isRoot ) {
  ...
  } // Davidson::runSparseMicro


  template <typename _F>
  bool Davidson<_F>::runMicro() {

    if(this->formSubspaceMatrix_)
      return runSparseMicro();
